...
function AddMemory() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !title || !description || images.length === 0) {
      setPopupMessage('Please fill out all fields and add at least one image.');
      setPopupType('error');
      setTimeout(() => {
        setPopupMessage('');
        setPopupType('');
    ...
    try {
      ...
      // ⚠️ Fallback: Optional - Save to localStorage if server fails
